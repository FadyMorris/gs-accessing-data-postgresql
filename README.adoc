...
:project_id: gs-accessing-data-postgresql
:java_version: 17
:build_system: gradle
:build_name: accessing-data-postgresql
:build_version: 0.0.1-SNAPSHOT
:network_container: guide-postgresql


This repository was forked from https://github.com/spring-guides/gs-accessing-data-mysql[Spring Guides - Accessing data with MySQL] repository. The code was modified to use PostgreSQL instead of MySQL. The database configuration is in  `.env` file.

This guide walks you through the process of creating a Spring application connected to a
PostgreSQL Database (as opposed to an in-memory, embedded database, which most of the other
...
You will create a PostgreSQL database, build a Spring application, and connect it to the newly
created database.
...
You can use this https://start.spring.io/#!type=maven-project&language=java&packaging=jar&groupId=com.example&artifactId=accessing-data-mysql&name=accessing-data-mysql&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.accessingdatamysql&dependencies=web,data-jpa,mysql,docker-compose,testcontainers,postgresql[pre-initialized project] and click Generate to download a ZIP file. This project is configured to fit the examples in this tutorial.
...
. Click *Dependencies* and select *Spring Web*, *Spring Data JPA*, *MySQL Driver*, *Docker Compose Support*, *Testcontainers*, and *PostgreSQL Driver*.
. Click *Generate*.
...
Before running the application, you also need to start the PostgreSQL container as Spring Boot's docker compose no longer does that for you:
...
For MySQL, When you are on a production environment, you may be exposed to SQL injection attacks. A
...
Congratulations! You have just developed a Spring application that is bound to a PostgreSQL
